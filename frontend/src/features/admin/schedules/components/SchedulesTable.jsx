    ...
    <>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-border">
              ...
              <th className="text-left py-3 px-4">
                <SortHeader label="Día" field="day" current={sortField} dir={sortDir} onSort={onSort} />
              </th>
              <th className="text-left py-3 px-4">
                <SortHeader label="Hora Inicio" field="startTime" current={sortField} dir={sortDir} onSort={onSort} />
              </th>
              <th className="text-left py-3 px-4">
                <SortHeader label="Hora Fin" field="endTime" current={sortField} dir={sortDir} onSort={onSort} />
              </th>
              <th className="text-left py-3 px-4">
          ...
          <tbody>
            {schedules.map((schedule) => (
              <tr key={schedule.id} className="border-b border-border hover:bg-accent/50 transition-colors">
                <td className="py-4 px-4">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-primary/10 rounded-full flex items-center justify-center">
                      <Clock className="h-5 w-5 text-primary" />
                    </div>
                    <span className="font-medium text-foreground">{schedule.barber}</span>
                  </div>
                </td>
                <td className="py-4 px-4 text-foreground">{schedule.day}</td>
                <td className="py-4 px-4 text-foreground">{schedule.startTime}</td>
                <td className="py-4 px-4 text-foreground">{schedule.endTime}</td>
                <td className="py-4 px-4">
                  <span
      ...
      <div className="flex items-center justify-between mt-4 pt-4 border-t border-border">
        <p className="text-sm text-muted-foreground">
          Mostrando {(currentPage - 1) * itemsPerPage + 1} - {Math.min(currentPage * itemsPerPage, filteredCount)} de {filteredCount} horarios
        </p>
