...
export default function BarberMobileDock({ pendingNoveltiesCount = 0, todayAppointmentsCount = 0 }) {
  const links = [
    ...
    { to: "/barbero/citas", label: "Citas", icon: Scissors, highlight: true, badge: todayAppointmentsCount > 0 ? todayAppointmentsCount : null },
    { to: "/barbero/novedades", label: "Novedades", icon: AlertCircle, badge: pendingNoveltiesCount > 0 ? pendingNoveltiesCount : null },
          ...
          if (link.highlight) {
            return (
              ...
              >
                ...
                {link.badge && (
                  <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full bg-destructive text-white text-[9px] font-black flex items-center justify-center animate-pulse">
                    {link.badge}
                  </span>
                )}
                <span className="sr-only">Mis Citas</span>
              </NavLink>
            );
          }
          ...
          return (
            ...
            >
              <div className="relative">
                ...
                {link.badge && (
                  <span className="absolute -top-1.5 -right-2 w-4 h-4 rounded-full bg-amber-500 text-black text-[9px] font-black flex items-center justify-center animate-pulse">
                    {link.badge}
                  </span>
                )}
              </div>
